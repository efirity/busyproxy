...
CODE="${APK_PROMO_CODE:-5409}"
URL="https://busyproxy.net/api/download/apk?code=${CODE}"
echo ""
echo "✓ Published (promo-gated)"
echo "  page:   https://busyproxy.net/#download"
echo "  api:    $URL"
echo "  promo:  ${CODE}  (env APK_PROMO_CODE)"
echo "  sha256: $SHA"
echo "  local:  $LOCAL_PUB/BusyProxy-latest-debug.apk"
echo ""
echo "Direct /downloads/*.apk without ?code= is blocked (403)."
echo "When Play is approved: set APP_DOWNLOAD_CHANNEL=\"play\" in src/data/app-download.ts"
echo "On phone: open #download → enter promo → Download → Install."